...
import co.killionrevival.curiosities.commands.ItemArguments;
...
public class RewardUtil {
    ...
    public static CycleReward getCycleRewardForModifier(
        ...
        final BigDecimal money = BigDecimal.valueOf(rewardsConfig.getDouble("money", 0.0));
    ...
    public static ItemStack buildItemStackFromRewardItem(final CycleItem item) {
        ...
        // Handle Curiosities items
        if (materialName.startsWith(CURIOSITIES_PREFIX)) {
            try {
                final String curiositiesName = materialName.substring(CURIOSITIES_PREFIX.length());

                // Create proper ItemArguments object
                ItemArguments itemArgs = new ItemArguments();
                if (item.isStamped()) {
                    itemArgs.setStamped(true);
                    itemArgs.setSuperStamped(false);
                }

                // Try to create the custom item
                CuriousItem customItem = CuriousItemFactory.createDefault(
                    new NamespacedKey(Curiosities.getNamespace(), curiositiesName.toLowerCase()),
                    itemArgs
                );

                if (customItem == null) {
                    ConsoleUtil.sendError("CycleItem reward: " + materialName + " has incorrect custom item name.");
                    return null;
                }
                ...
                // Curiosities library is not available
                ConsoleUtil.sendError("Curiosities plugin is not available: " + e.getMessage());
                return null;

            } catch (Exception e) {
                // Any other exception
                ConsoleUtil.sendError("Error creating Curiosities item: " + e.getMessage());
                e.printStackTrace();
                return null;
            }
        }
        ...
        try {
            ...
            if (material == null) {
                ConsoleUtil.sendError("CycleItem reward: " + item.getMaterial() + " has incorrect material name.");
                return null;
            }
            ...
            item.getEnchantments().forEach((key, value) -> {
                try {
                    ...
                    ConsoleUtil.sendError("Error adding enchantment " + key + ": " + e.getMessage());
                }
            });

            // Safely handle stamping if needed
            if (item.isStamped()) {
                try {
                    ItemStackUtil.stampItemStack(itemStack, false, null);
                } catch (NoClassDefFoundError e) {
                    ConsoleUtil.sendDebug("Curiosities not available for stamping. Skipping stamp.");
                } catch (Exception e) {
                    ConsoleUtil.sendDebug("Error while stamping item: " + e.getMessage());
                }
            }

            return itemStack;
        } catch (Exception e) {
            ConsoleUtil.sendError("Unexpected error creating item " + materialName + ": " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    public static List<Component> getRewardLore(
